...
def load_data(file, device):
    ...
    train_X, val_X, test_X = map(lambda x: (x - mean) / std, [train_X, val_X, test_X])
    train_X, val_X, test_X = map(lambda x: torch.tensor(x, dtype=torch.float32, device=device),[train_X, val_X, test_X])
    train_y, val_y, test_y = map(lambda x: torch.tensor(x, dtype=torch.float32, device=device), [train_y, val_y, test_y])
...
if __name__ == '__main__':
    ...
    dataset, val_X, val_y, test_X, test_y = load_data(file='./data/peak_pressure_time_neg.npy', device=device)
    print(len(val_X))
